...
class Dataset:
    def __init__(self, conf):
        ...
        self.depths_np = np.stack([(256 - cv.imread(im_name, cv.IMREAD_GRAYSCALE) ) for im_name in self.depths_lis]) / 256.0
    ...
    def gen_rays_at(self, img_idx, resolution_level=1):
        ...
        return rays_o.transpose(0, 1), rays_v.transpose(0, 1), depth.transpose(0, 1)
    ...
    def gen_random_rays_at(self, img_idx, batch_size):
        ...
        depth = self.depth_images[img_idx][(pixels_y, pixels_x)] #batch_size. 1
        ...
        return torch.cat([rays_o.cpu(), rays_v.cpu(), color, mask[:, :1]], dim=-1).cuda(), depth.cuda()    # batch_size, 10
